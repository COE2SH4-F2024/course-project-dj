...
bool Player::checkFoodConsumption(Food* food)
{
    ...
    objPos head = playerPosList->getHead();

    for (int i = 0; i < foodBucket->getSize(); ++i)
    {
        objPos foodItem = foodBucket->getElement(i);
        if (head.isPosEqual(&foodItem))
        {
            // Apply effect based on food symbol
            if (foodItem.getSymbol() == 'F') // Regular food
            {
                mainGameMechsRef->incrementScore(10);
            }
            else if (foodItem.getSymbol() == 'S') // Special food
            {
                mainGameMechsRef->incrementScore(50);
                increasePlayerLength(10); // Grow snake by 10 units
            }

            // Remove the consumed food from the bucket
            foodBucket->removeAt(i);
            return true;
        }
    }
    return false;
...
void Player::increasePlayerLength(int length)
{
    ...
    {
        objPos tail = playerPosList->getTail();
        playerPosList->addToTail(tail); // Extend snake by duplicating tail
    }
}
